...
CMyMatrix CMyMatrix::invers()
{
    ...
    if (!(this->getZ() == 2 && this->getS() == 2)) {
        wcerr << "Inverse Matrix kann nicht berechnet werden (nur 2x2 Matrix möglich)" << endl; exit(0);
    }
    else {
...
CMyVektor CMyMatrix::operator*(CMyVektor x)
{
    if (!(this->getS() == x.GetDimension())) {
        ...
        wcerr << "Matrix-Vektor Multiplikation nicht möglich (Dimensionen stimmen nicht)" << endl; exit(0);
    }
    else {
