...
bool EdgeWeightedDigraph::del_Edge(DirectedEdge e)
{
	int u = e.from(),
		v = e.to();
	std::vector<DirectedEdge> newV;
	bool smallerList = (newV.size() < adj[u].size());

	for (int i = 0; i < adj[u].size(); i++) // alle Edges ohne e in neuem Vektor
		if (adj[u][i].to() != v)
			newV.push_back(adj[u][i]);

	adj[u].clear(); adj[u] = newV;

	return smallerList; // true wenn newV kleiner
}
