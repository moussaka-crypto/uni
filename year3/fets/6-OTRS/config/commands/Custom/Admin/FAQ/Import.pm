...
package Kernel::System::Console::Command::Custom::Admin::FAQ::Import;
...
sub Run {
    ...
    for my $RowRef ( @{$DataRef} ) {
        ...
        foreach my $ID (@IDs) {
            ...
            if ( $FAQ{Title} eq $Title ) {
                $Self->Print("A FAQ article with title \'$Title\' already exists.\n");
                $Self->PrintError("Can't add FAQ article.\n");
                return $Self->ExitCodeError();
            }
