...
package Kernel::System::Console::Command::Custom::Admin::Group::UserLink;
...
sub Run {
    ...
    for my $Permission (qw(ro move_into create owner priority rw)) {
        $Permissions{$Permission} = ( $Self->GetOption('permission') =~ m/$Permission/ ) ? 1 : 0
    }
    ...
    {
        $Self->PrintError("Can't link user to group.");
