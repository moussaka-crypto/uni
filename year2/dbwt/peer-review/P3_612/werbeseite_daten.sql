...
SELECT UNIQUE(typ) FROM allergen;
...
INSERT INTO gericht VALUES (21, 'Currywurst mit Pommes', 'Der Klassiker, Currywurst mit Bommes', '2022-11-15', 0, 0, 2.50, 3.50);
INSERT INTO gericht_hat_kategorie VALUES (21, 3);


SELECT gericht.name Name, GROUP_CONCAT(gericht_hat_allergen.code SEPARATOR ',') Allergen, gericht.preis_intern, gericht.preis_extern
...
SELECT GROUP_CONCAT(CONCAT_WS('-',allergen.code, allergen.name) SEPARATOR ',') Allergencodes
...
SELECT gericht.name, allergen.code, allergen.name, allergen.typ
FROM gericht, allergen, gericht_hat_allergen
where gericht_hat_allergen.code = allergen.code
  and gericht_hat_allergen.gericht_id = gericht.id;

SELECT g.name as Gericht, group_concat(a.name) as Allergen
from gericht_hat_allergen
         join gericht g on g.id = gericht_hat_allergen.gericht_id
         join allergen a on gericht_hat_allergen.code = a.code
group by Gericht;


-- Ändern Sie die vorherige Abfrage so ab,
-- dass alle existierenden Gerichte dargestellt werden
-- (auch wenn keine Allergene enthalten sind).
SELECT g.name as Gericht, a.name as Allergen
from gericht_hat_allergen
         right join gericht g on g.id = gericht_hat_allergen.gericht_id
         left join allergen a on gericht_hat_allergen.code = a.code;

-- Ändern Sie die vorherige Abfrage so ab,
-- so dass im Ergebnis alle existierenden Allergene dargestellt werden
-- (auch wenn diese nicht einem Gericht zugeordnet sind).
SELECT g.name as Gericht, a.name as Allergen
from gericht_hat_allergen
         right join allergen a on a.code = gericht_hat_allergen.code
         left join gericht g on g.id = gericht_hat_allergen.gericht_id;

-- Die Anzahl der Gerichte pro Kategorie aufsteigend sortiert nach Anzahl.
SELECT k.name, count(gericht_hat_kategorie.kategorie_id) as Anzahl
FROM gericht_hat_kategorie
         join kategorie k on gericht_hat_kategorie.kategorie_id = k.id
group by kategorie_id
order by Anzahl desc;


-- Ändern Sie die vorherige Abfrage so ab,
-- dass dabei nur die Kategorien dargestellt werden,
-- die mehr als 2 Gerichte besitzen.
SELECT k.name, count(gericht_hat_kategorie.kategorie_id) as Anzahl
FROM gericht_hat_kategorie
         join kategorie k on gericht_hat_kategorie.kategorie_id = k.id
group by kategorie_id
